"""Export swarm-agency agents to Windsurf rules format.

Produces a single .windsurfrules file combining all agents as sections.
"""
...
def export_windsurf(agents_dir: str, output_path: str) -> str:
    """Export all agent markdown files to a single .windsurfrules file.

    Args:
        agents_dir: Path to the agents/ directory containing department subdirs.
        output_path: Path to write the .windsurfrules file.
    ...
    sections.append("# Windsurf Rules — Swarm Agency Agent Personas")
    sections.append("")
    sections.append(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export agents to Windsurf format")
    parser.add_argument(
        "--agents-dir",
        default="agents",
    ...
    parser.add_argument(
        "--output-path",
        default="output/.windsurfrules",
        help="Output file path (default: output/.windsurfrules)",
    )
    args = parser.parse_args()

    result = export_windsurf(args.agents_dir, args.output_path)
